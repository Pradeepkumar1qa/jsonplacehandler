...
function colorgenrator(){
  return cardcolorgenrator[Math.floor(Math.random()*3)]
}
...
function genratecard(user){
  ...
  for(i in Allcardobjectinformation){
    if(Allcardobjectinformation[i].id===globalresult[user].id){
      console.log("card is all ready there with user id "+i)
      return;
    }
...
function cardanimation(counter){
  ...
  Allcard[counter-1].getElementsByTagName('b')[3].innerHTML=Allcardobjectinformation[counter-1].body
}
